...
US3 は US1（実候補抽出）と US2（実クリック実行）を包括した統合フローとして扱い、オーバーレイラベルは「UI部品名」ではなく「対象候補の座標位置に重畳する操作ラベル」を指す。
...
- US3の必須達成条件を `spec.md` の FR-005/FR-006/FR-010 と User Story 3 受け入れ条件へ再整合した。
- オーバーレイラベルは、US1で得た実候補の座標に重ねる表示であることを明確化した。
- US3チェックポイントは「実候補表示→ラベル入力選択→US2クリック実行→セッション終了→再起動可能」を通しで満たすことに固定した。
- `tasks.md` で US3 の未達タスク（T032〜T036）を再オープンし、統合不足を埋めるタスク（T042〜T046）を追加した。
