...
2. `Cmd+Shift+M` を押して候補表示（オーバーレイ）を確認する。
3. オーバーレイ上に候補ラベル（`AA` など）が実座標に重畳表示されることを確認する。
4. ラベル入力で候補が絞り込まれることを確認する。
5. `Enter` で左クリック、`Ctrl+Enter` で右クリックが実行されることを確認する。
6. 実行後にオーバーレイが閉じ、下部ステータスに `再実行可能` が表示されることを確認する。
7. 同じ手順を再実行し、連続で起動できることを確認する。

### US3 期待結果
- 開始から終了まで1回の流れが異常終了せず完走する。
- 候補ラベルは画面要素候補の座標位置に表示される。
- `Enter`（left）/`Ctrl+Enter`（right）でクリック種別を切り替えられる。
- キャンセル時も終了処理が実行される。
- 完了後に次回ホットキーを受け付ける。

## 最新通しフロー（US1→US2→US3）
1. `SCREENSEARCH_ACCESSIBILITY_TRUSTED=true dotnet run` で起動する。
2. `Cmd+Shift+M` を押し、候補抽出ログ（`Elements Extracted`）を確認する。
3. ラベル入力で候補を1件へ絞り込む。
